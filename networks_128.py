...
class Generator:
    ...
    def __call__(self, inputs, train_phase, y, nums_class):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            inputs = G_Resblock("ResBlock1", inputs, 1024, train_phase, y, nums_class)
            inputs = G_Resblock("ResBlock2", inputs, 512, train_phase, y, nums_class)
            inputs = G_Resblock("ResBlock3", inputs, 256, train_phase, y, nums_class)
            inputs = G_Resblock("ResBlock4", inputs, 128, train_phase, y, nums_class)
            inputs = G_Resblock("ResBlock5", inputs, 64, train_phase, y, nums_class)
